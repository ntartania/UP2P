...
<xsl:template match="/community">
	...
	<div class="link-community">
		...
		<tr><td clas="title"><strong>Title</strong></td><td><strong>Year</strong></td><td><strong>Remove</strong></td></tr>
		<xsl:apply-templates>
			<xsl:sort select="./film/initial_release_date" order="descending" />
		</xsl:apply-templates>
		</table>
	</div>
</xsl:template>
...
<xsl:template match="/community/resource">
	...
	<!-- Year -->
	<td><xsl:value-of select="./film/initial_release_date" /></td>
