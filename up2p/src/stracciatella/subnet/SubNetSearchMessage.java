...
public class SubNetSearchMessage extends Message {
	...
	public final static byte SUBNETQUERY = (byte)0x82;
	 ...
	 *
	 */
	public SubNetSearchMessage(byte[] rawMessage, Connection originatingConnection) {
	...
	void buildPayload() {
		...
		if (searchType == GET_BY_NAME) {
			...
			payload = new byte[criteriaChars.length + SUB_NET_ID_LENGTH + 8];
			
		} else if (searchType == GET_BY_HASH) {
			// +3 Bytes for minimum speed, and null following criteria
			payload = new byte[criteriaChars.length + SUB_NET_ID_LENGTH + 3];
